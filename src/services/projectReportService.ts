 ...
 */
function addProjectDetails(doc: jsPDF, project: Project) {
  const startY = 60;
 ...
 */
function addClientInformation(doc: jsPDF, project: Project) {
 ...
 */
async function addLayoutsSection(doc: jsPDF, layouts: Layout[], layoutImages: { [key: string]: string }) {
  const startY = (doc as any).lastAutoTable.finalY + 15;
  ...
  if (startY > 220) {
    ...
    doc.setFontSize(16);
    doc.setTextColor(0, 51, 102);
    doc.text("Project Layouts", 15, 20);
    
    await addLayoutDetails(doc, layouts, layoutImages, 30);
  } else {
    // Section title
    doc.setFontSize(16);
    doc.setTextColor(0, 51, 102);
    ...
    await addLayoutDetails(doc, layouts, layoutImages, startY + 10);
  }
}
 ...
 */
async function addLayoutDetails(doc: jsPDF, layouts: Layout[], layoutImages: { [key: string]: string }, startY: number) {
  let currentY = startY;
 ...
 */
function addCalculationsSection(doc: jsPDF, calculations: any[]) {
  // Check if we need to add a new page
  const startY = (doc as any).lastAutoTable?.finalY + 15 || 220;
  
  if (startY > 220) {
    doc.addPage();
    doc.setFontSize(16);
    doc.setTextColor(0, 51, 102);
    doc.text("Project Calculations", 15, 20);
    
    addCalculationDetails(doc, calculations, 30);
  } else {
    // Section title
    doc.setFontSize(16);
    doc.setTextColor(0, 51, 102);
    ...
    addCalculationDetails(doc, calculations, startY + 10);
  }
}
 ...
 */
function addCalculationDetails(doc: jsPDF, calculations: any[], startY: number) {
  let currentY = startY;
  ...
  for (let i = 0; i < calculations.length; i++) {
    ...
    if (calculation.results) {
      ...
      if (calculation.results.cost) {
        ...
        doc.text(`Total Project Cost: ${formatCurrency(calculation.results.cost.totalProjectCost)}`, 15, currentY);
        currentY += 5;
